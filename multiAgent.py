...
class GameState:
	...
	def getLegalTargetActions(self):
...
class TargetRules:
	...
	def getLegalActions(state):
		...
		"""
		return Actions.getPossibleActions(state.getTargetState().configuration, state.data.layout.obstacles)
	...
	def applyAction(state, action):
		...
		targetState.configuration = targetState.configuration.generateSuccessor(vector)
...
class PursuerRules:
	...
	def applyAction(state, action, pursuerIndex):
		...
		pursuerState.configuration = pursuerState.configuration.generateSuccessor(vector)

	applyAction = staticmethod(applyAction)

	def collide(state, pursuerState, agentIndex):
...
def readCommand():
	...
	args["target"] = targets.LeftTurnTarget()
	# creat several pursuers
	args["pursuers"] = [pursuers.RandomPursuer(i) for i in range(1,3)]
